...
class Tags
{
    ...
    /**
     * PEER_* tags can be emitted by either client-side of server-side to
     * describe # the other side/service in a peer-to-peer communications,
     * like an RPC call.
     */
    ...
    /** PEER_ADDRESS (string) suitable for use in a networking client library.  # This may be a "ip:port", a bare "hostname", a FQDN, or even a # JDBC substring like "mysql://prod-db:3306" */
    ...
    /**
     * HTTP_URL (string) should be the URL of the request being handled in
     * this # segment of the trace, in standard URI format. The protocol is
     * optional.
     */
    const HTTP_URL = 'http.url';

    /** HTTP_METHOD (string) is the HTTP method of the request. Both upper/lower case values are allowed. */
    const HTTP_METHOD = 'http.method';

    /** HTTP_STATUS_CODE (int) is the numeric HTTP status code (200, 404, etc) of the HTTP response. */
    const HTTP_STATUS_CODE = 'http.status_code';

    /** DATABASE tags */

    /**
     * DATABASE_INSTANCE (string) The database instance name. E.g., In java,
     * if the jdbc.url="jdbc:mysql://127.0.0.1:3306/customers", the instance
     * name is "customers"
     */
    ...
    /** DATABASE_TYPE (string) For any SQL database, "sql". For others, the lower-case database category, e.g. "cassandra", "hbase", or "redis". */
    const DATABASE_TYPE = 'db.type';

    /** DATABASE_USER (string) Username for accessing database. E.g., "readonly_user" or "reporting_user" */
